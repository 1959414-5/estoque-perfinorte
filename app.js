...
function filtrarPickerPeca() {
  ...
  filtradas.slice(0, 60).forEach(p => {
    ...
    div.innerHTML = thumbHtml(p.Imagem_URL) +
      ...
      '<div class="picker-item-sub">' + esc(p.MP || '') + (p.Espessura ? ' · ' + esc(p.Espessura) : '') + ' · ' + esc(p.Linha || '') + '</div>' +
      '</div>';
...
function selecionarPeca(p) {
  ...
  document.getElementById('peca-picker-sub').textContent =
    (p.MP || '') + (p.Espessura ? ' · ' + p.Espessura : '') + (p.Linha ? ' · ' + p.Linha : '');
}
...
function renderCatalogoLista() {
  ...
  filtradas.forEach(p => {
    ...
    div.innerHTML =
      ...
      '<div class="catalog-card-line">' + esc(p.MP || '—') + (p.Espessura ? ' · ' + esc(p.Espessura) : '') + '</div>' +
...
const LABEL_TELEFONE = '(44) 3366-1415';
 ...
 try {
  ...
  const labelsHtml = lista.map((p, i) => {
    ...
    return '<div class="label">' +
      ...
      (p.MP ? '<div class="label-meta">MP: ' + esc(p.MP) + (p.Espessura ? ' de ' + esc(p.Espessura) : '') + '</div>' : '') +
